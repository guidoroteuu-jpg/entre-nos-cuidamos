      ...
      <div className="w-full max-w-lg">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">30 dias grátis, sem cartão de crédito</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-card rounded-2xl p-6 border border-border shadow-elevated space-y-4">
          <div>
            ...
            <Input value={form.nome} onChange={(e) => handleChange("nome", e.target.value)} placeholder="Escola Municipal..." className="h-11" />
          </div>
          <div>
            ...
            <Input value={form.cnpj} onChange={(e) => handleChange("cnpj", e.target.value)} placeholder="00.000.000/0000-00" className="h-11" />
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">Nome do responsável *</label>
            <Input value={form.responsavel} onChange={(e) => handleChange("responsavel", e.target.value)} placeholder="Seu nome completo" className="h-11" />
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">Email institucional *</label>
            <Input type="email" value={form.email} onChange={(e) => handleChange("email", e.target.value)} placeholder="contato@escola.edu.br" className="h-11" />
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">Telefone</label>
            <Input value={form.telefone} onChange={(e) => handleChange("telefone", e.target.value)} placeholder="(11) 99999-9999" className="h-11" />
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">Número estimado de alunos</label>
            <Input type="number" value={form.alunos} onChange={(e) => handleChange("alunos", e.target.value)} placeholder="500" className="h-11" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">Cidade *</label>
              <Input value={form.cidade} onChange={(e) => handleChange("cidade", e.target.value)} placeholder="São Paulo" className="h-11" />
            </div>
            <div>
          ...
          <Button type="submit" variant="hero" className="w-full h-12 text-base" disabled={loading}>
            {loading ? "Criando sua conta..." : "Começar 30 dias grátis"}
          </Button>
        </form>
